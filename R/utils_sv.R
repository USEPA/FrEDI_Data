  ...
  x_popProj <- c_regions |> map(function(region_i){
    ...
    regionPop_i <- states_i |> map(function(state_j){
      ...
      statePop_j    <- geoids_j |> map(function(geoid_k){
        ...
        has_k <- !(fun_k |> is.null())
        if(has_k) {y_k <- fun_k(years_i)} else {y_k <- NA}
...
calc_tractScaledImpacts <- function(
  ...
  years_x  <- driverValues[["year"]]
  values_x <- driverValues[[xCol  ]]
...
calc_tractImpacts <- function(
  ...
  } else if(sector=="Air Quality - Premature Mortality") {
    c_svNACols <- c("sv_plus65")
  } ### End else if
  c_svGroupCols  <- svGroups[svGroups %in% (svInfo |> names())]
  ...
  svGroupCols <- svGroups[svGroups %in% (svInfo |> names())]
  ...
  x_impacts   <- svInfo |> mutate(none = 1) |> select(-all_of(c_dropCols0))
  ...
  x_impacts   <- x_impacts |> left_join(popData, by = c(c_joinCols0))
  ...
  x_impacts   <- x_impacts |> select(-all_of(c_weightCols))
  ...
  x_impacts    <- x_impacts |> mutate_at(vars(c_svNACols), function(z){0})
  rm(c_svNACols)
  ...
  x_impacts       <- x_impacts |> select(-c(cutoffNat0));
  rm(cutoffNat0)
  # Sys.sleep(sleep)
  ...
  ### Columns
  groupsReg0      <- groupsNat0 |> c("region")
  ...
  if(!.testing){
    ...
    x_impacts    <- x_impacts |> select(c(select0))
