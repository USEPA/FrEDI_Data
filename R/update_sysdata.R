...
update_sysdata <- function(
  ...
  ###### Directories ######
  projectPath <- is.null(projectPath) |> ifelse(".", projectPath)
  outPath     <- is.null(outPath) |> ifelse(system.file(package="FrEDI"), outPath); 
  ...
  ###### Update file paths
  ### Check file exists
  new_ext       <- ((rDataExt |> tolower()) == "rds") |> ifelse("rda", rDataExt)
  ...
  if(sv){
    for(i in 1:nrow(df_sv)){
      ...
      if(save){
        ...
        pattern_sysdata <- names_sysdata |> paste(collapse = "|")
