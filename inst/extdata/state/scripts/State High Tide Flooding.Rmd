...
for (i in htf_files){
  df_i <- dataPath %>% file.path(i) %>%
          ...
          mutate(variant = str_extract(i, "(?<=-)(.*?)(?=-)"),
                 slr = case_when(
                   str_extract(i, "([^\\-]+)(?=\\.csv$)") == "low" ~ "30",
                   str_extract(i, "([^\\-]+)(?=\\.csv$)") == "intlow" ~ "50",
                   str_extract(i, "([^\\-]+)(?=\\.csv$)") == "int" ~ "100",
                   str_extract(i, "([^\\-]+)(?=\\.csv$)") == "inthigh" ~ "150",
                   str_extract(i, "([^\\-]+)(?=\\.csv$)") == "high" ~ "200",
                   str_extract(i, "([^\\-]+)(?=\\.csv$)") == "extreme" ~ "250"
                 ))
...
htf_df <- htf_df %>%
  ...
  select(county, ST_fips, variant, slr, all_of(years))
...
htf_df_state <- htf_df %>%
  select(-county) %>%
  group_by(ST_fips, variant, slr) %>%
  summarize_all(sum, na.rm = TRUE)

htf_smooth <- htf_df_state %>%
  pivot_longer(
    cols = -c(ST_fips, variant, slr),
    names_to = 'year',
    values_to = 'value1'
  ) %>%
  arrange(ST_fips, variant, slr, year) %>%
  group_by(ST_fips, variant, slr) %>%
  ...
  ungroup %>%
  select(ST_full, ST_postal, variant, impactType, year, slr, value) %>%
