...
mh_pop_multiplier_df <- mh_future_df_statesum %>%
  left_join(FrEDI_pops, by = c("ST" = "ST_postal", "YEAR_POP" = "Year")) %>%
...
mh_pop_multiplier_df_full <- mh_pop_multiplier_df
for (i in mh_pop_multiplier_df$state %>% unique){
  df_state_11 <- mh_pop_multiplier_df %>%
    filter(state == i & year == 2011)
  df_state_13 <- mh_pop_multiplier_df %>%
    filter(state == i & year == 2013)

  df_state_10       <- df_state_13
  df_state_10$value <- df_state_11$value - ((df_state_13$value - df_state_11$value)/(df_state_13$year - df_state_11$year))
  df_state_10$year  <- 2010
  mh_pop_multiplier_df_full <- mh_pop_multiplier_df_full %>% bind_rows(df_state_10)
}
mh_pop_multiplier_df_full <- mh_pop_multiplier_df_full %>% arrange(year)

if (saveFiles){
  mh_pop_multiplier_df_full %>% write.csv(file.path(outPath, "Scalar_physAdj_MH_popSliceAdj.csv"), row.names = F)
}
```
...
all(mh_pop_multiplier_df_full$state %in% state_crosswalk$ST_full) && all(state_crosswalk$ST_full %in% mh_pop_multiplier_df_full$state)
sum(mh_pop_multiplier_df_full$value >= 1) == 0
```
...
all_state_df_full <- all_state_df
for (i in all_state_df$state %>% unique){
  df_state_11 <- all_state_df %>%
    filter(state == i & year == 2011)
  df_state_13 <- all_state_df %>%
    filter(state == i & year == 2013)

  df_state_10       <- df_state_13
  df_state_10$value <- df_state_11$value - ((df_state_13$value - df_state_11$value)/(df_state_13$year - df_state_11$year))
  df_state_10$year  <- 2010
  all_state_df_full <- all_state_df_full %>% bind_rows(df_state_10)
}
all_state_df_full <- all_state_df_full %>% arrange(year)

if (saveFiles){
  all_state_df_full %>% write.csv(file.path(outPath, "Scalar_damageAdj_MH_popDemoAdj.csv"), row.names = F)
}
```

```{r MH_popDemoAdj qc check}
all(all_state_df_full$state %in% state_crosswalk$ST_full) && all(state_crosswalk$ST_full %in% all_state_df_full$state)
```