...
configureFrediData <- function(
  ...
  module      = "fredi"    , ### Module directory relative to dataDir
  # scenarioDir = "scenarios", ### Directory to scenario relative to dataDir
  outDir      = "." |> file.path("data"), ### Directory to save outputs relative to projectDir
  ### Info on config file
  ...
  ### Info on saving
  outFile     = "frediData" |> paste0(".", "rda" ), ### Name of file to which to save results
  ...
  msg1 <- msg0 + 1
  msg2 <- msg0 + 2
  msg0 |> get_msgPrefix(newline=T) |> paste0("Running configureFrediData()...") |> message()
  # msg1 <- msg0 |> paste0("\t")
  # msg2 <- msg1 |> paste0("\t")
  ...
  if(return) return(list_systemData0)
  # return(returnList)
}
